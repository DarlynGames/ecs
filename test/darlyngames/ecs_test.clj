(ns darlyngames.ecs-test
  (:require [clojure.test :refer :all]
            [darlyngames.ecs :refer :all]))
...
(deftest ces
  ...
  (testing "I haz component?"
    (let [e-id (create-entity)
          system (-> (create-system)
                     (add-component e-id (->TestComponent :foo)))]
      (is (has-component? system e-id TestComponent))
      (is (not (has-component? system e-id OtherTestComponent)))))
  )
